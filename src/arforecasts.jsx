...
function ARforecast() {
    ...
    const fetchData = async () => {
        try {
            ...
            const response = await fetch(`http://127.0.0.1:5000/get_ar_forecasts/${jwt.iduser}`, { method: "GET", headers: headers });
            if (response.ok) {
    ...
    const chartactData = actualData.map(item => item.total_sales);
    ...
    return (
        <div>
            <h5>Accounts Recievable Forecast</h5>
